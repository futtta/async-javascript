    ...
    jQuery.post(aj_localize_admin.ajaxurl, data, function(response) {
      try {
        ...
        if (response.status !== false) {
          ...
          var flt = parseFloat(
            response.results.fully_loaded_time / 1000
          ).toFixed(2);
          ...
          jQuery("#aj_gtmetrix_baseline_pagespeed").html(
            baseline_pagespeed + "%"
          );
...
/*   jQuery(document).on(
  ...
  jQuery(document).on("click", ".aj_steps_button", function(e) {
    ...
    if (theStep == "aj_goto_settings") {
      ...
      var aj_autoptimize_method = jQuery(
        "input[type=radio][name=aj_autoptimize_method]:checked"
      ).val();
...
/* Add notify settings in its own global function so that it can easily be
Modified and changed. */
function notifySettingsSaved() {
  jQuery("#aj_notification")
    .fadeIn("slow")
    .html(
      "Settings Saved <span class=\"aj_dismiss\"><a title=\"dismiss this notification\">x</a></span>"
    )
    .delay(500)
    .fadeOut("slow");
}
